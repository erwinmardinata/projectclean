...
class Surat_keluar extends Back_controller {

	public $judul = "Surat Keluar";
	public $url = "Surat_keluar";

	function __construct() {

		parent::__construct();
		$this->load->model('Msurat_keluar', 'mdl');
	...
	function disposisi_insert() {
		...
		redirect($this->url.'/detail/'.$post['id_Surat_keluar_masuk']);
	...
	function update() {
		...
		if(empty($media)) {
			...
			if(!$this->upload->do_upload('lampiran')) {
				$error = $this->upload->display_errors();
				$this->alert_error($error);
				redirect($url.'edit/'.$post['id_Surat_keluar_masuk']);
			}
			$get_name = $this->upload->data();
			$name = $get_name['file_name'];
	...
	function get_data() {
		...
		foreach($fetch_data as $row)
		{
                  if($row->lampiran != NULL) {
	                $lam = '<a target="_blank" href="'.base_url('assets/lampiran/'.$row->lampiran).'" class="btn btn-primary">Lihat</a>';
                  } else {
                  	$lam = "<i>Tidak ada lampiran</i>";
                  }
	...
	function print_Surat_keluar($id) {
		...
		$pdf->SetAuthor('LPKSI');
		$pdf->SetTitle('Surat_keluar LPKSI');
		...
		$html = $this->load->view('Surat_keluar', $data_array, true);

		$pdf->writeHTMLCell(0, 0, '', '', $html, 0, 1, 0, true, '', true);


		$pdf->Output("Surat_keluar_keterangan_usaha.pdf", 'I');
